...
\begin{frame}{Suppose an optimal $\Qq(t)$ exists:}
  \begin{figure}[h]
    \centering
    \begin{tikzpicture}[scale=1]
      ...
      \node[left,xshift=-4pt] (q0) at (0,\qs) {$\Qq(t_0)$};
      \node[left,xshift=-4pt] (qf) at (0,\qf) {$\Qq(t_f)$};
      \draw[dotted] (0,\qs) -- (y0);
      \draw[dotted] (0,\qf) -- (y1);
      \draw (-.08,\qs) -- (.08,\qs);
      \draw (-.08,\qf) -- (.08,\qf);
...
\begin{frame}{Suppose an optimal $\Qq(t)$ exists:}
  \begin{figure}[h]
    ...
    \begin{tikzpicture}[scale=1.15]
      ...
      % Perturbation
      \draw[domain=1.5:5.5, smooth, variable=\x, dashed, red] plot ({\x-.5}, {-1.7 + \x * (3.31667 + (-0.9 + 0.0833333 * \x) * \x) + \epsilon*((.5+2*sin(2*\x-2) * exp(2-1 * \x)) * (\x - 5.5) * (\x - 1.5))^2});
    ...
    \end{tikzpicture}
    \caption{Add a small perturbation ${\color{red}\varepsilon \cdot \bm \eta(t)}$}
  \end{figure}
\end{frame}
