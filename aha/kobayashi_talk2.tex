...
\begin{frame}{My perspective}
  \begin{itemize}
  \item My background in math\pause
    \begin{itemize}
      \item Weakest subject in high school (until Calculus)\pause
      \item Little exposure to higher math\pause
      \item Did like Physics, though!\pause
    \end{itemize}
  \item Former perspectives on ``Algebra'' \& Geometry\pause
    \begin{itemize}
    \item Algebra: abstract, opaque, uninspired\pause
    \item Geometry: tangible, intuitive, clear\pause
    \end{itemize}
...
\begin{frame}{Evolution of perspective}{}
  ...
  \end{itemize}
\end{frame}
% \begin{frame}{Unsatisfying!}{}
%   \begin{itemize}
%     \item What was I missing?
%       \begin{itemize}
%         \item Induction
%       \end{itemize}
%     \item Trusting vs.\ believing a result
%   \end{itemize}
% \end{frame}
...
\begin{frame}{Challenge}
  ...
  \begin{aquote}{My Neighbor}
    Can you do it with Calculus? Because \textbf{then} I'd be impressed.
  \end{aquote}
  \vfill
\end{frame}
\section{Aha!}
...
\begin{frame}
  \begin{figure}[H]
    ...
    \begin{tikzpicture}[every edge quotes/.append style={auto, text=blue}]
      ...
      \coordinate (ap) at (-\cubex, 0, 0);

      \draw[draw=blue, every edge/.append style={draw=blue, densely dashed, opacity=.5}, fill=green, fill opacity=.5]
      (ap)
      -- ++(0,0,\dx) coordinate (l)
      -- ++(\cubex, 0, 0) coordinate (m)
      -- ++(0,-\cubex,0) coordinate (n)
      -- ++(-\cubex, 0, 0) coordinate (p)
      -- (l) -- cycle;

      \draw[draw=blue, every edge/.append style={draw=blue}, fill=green, fill opacity=.5]
      (n)
      -- ++(0,0,-\dx) coordinate (np) -- ++(0,\cubex, 0) -- ++(-\cubex,0,0) -- ++(0,0,\dx) -- ++(\cubex, 0,0) edge ++(0,0,-\dx) -- ++(0,-\cubex,0);

      \coordinate (opp) at (0,0,0);

      \draw[draw=blue, every edge/.append style={draw=blue, densely dashed, opacity=.5}, fill=green, fill opacity=.5]
      (opp)
      -- ++(\dx,0,0) coordinate (q)
      -- ++(0, 0, -\cubex) coordinate (r)
      -- ++(0,-\cubex,0) coordinate (s)
      -- ++(0, 0, \cubex) coordinate (t)
      -- (q) -- cycle;

      \draw[draw=blue, every edge/.append style={draw=blue}, fill=green, fill opacity=.5]
      (t)
      -- ++(-\dx,0,0) coordinate (np) -- ++(0,\cubex, 0) -- ++(0,0,-\cubex) -- ++(\dx,0,0) -- ++(0, 0,\cubex) edge ++(0,0,-\dx) -- ++(0,-\cubex,0);

      \draw[draw=blue, densely dotted, opacity=.5] (s) -- ++(-\dx,0,0);
